...
async function renderPanelClip(panel, opts) {
  ...
  if (panel.audioPath && fs.existsSync(panel.audioPath)) {
    ...
    if (audioDur) dur = audioDur + (typeof breathPadding === "number" ? breathPadding : 0.35); // configurable small breath after narration
  }
...
async function concatClips(clips, transitions, finalPath, meta, fps, log) {
  ...
  if (clips.length === 1) {
    try {
      await runFfmpegWithContext(["-i", clips[0].file, ...compressionArgs(meta), finalPath], { stage: "concat", input: clips[0].file, output: finalPath }, log);
      return finalPath;
    } catch (e) {
      throw e;
    }
  ...
  if (!useXfade) {
    ...
    try {
      await runFfmpegWithContext(["-f", "concat", "-safe", "0", "-i", listFile, ...compressionArgs(meta), finalPath], { stage: "concat", input: listFile, output: finalPath }, log);
      return finalPath;
    } catch (e) {
      throw e;
    }
...
async function applyOverlayLogo(input, overlayFile, overlay, output, meta, log, width, height) {
  const W = Number(width) || DEFAULT_W;
  const H = Number(height) || DEFAULT_H;
...
async function normalizeAudio(input, output, meta, log) {
  try {
    await runFfmpegWithContext(["-i", input, "-af", "loudnorm=I=-16:TP=-1.5:LRA=11", ...compressionArgs(meta), output], { stage: "normalize", input, output }, log);
    return output;
  } catch (e) {
    throw e;
  }
