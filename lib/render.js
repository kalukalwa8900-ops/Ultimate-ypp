...
async function renderPanelClip(panel, opts) {
  ...
  if (panel.audioPath && fs.existsSync(panel.audioPath)) {
    ...
    if (audioDur) dur = audioDur + 0.35; // small breath after narration
  }
  ...
  let pre = `[0:v]${crop ? crop + "," : ""}${fitFilter(fit)}`;
  chain.push(`${pre}[fit]`);
  chain.push(`[fit]${motionFilter(panel.motion, clipDur, fps)},format=yuv420p,trim=duration=${clipDur},setpts=PTS-STARTPTS[base]`);
