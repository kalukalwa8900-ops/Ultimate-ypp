...
router.post("/assets/vfx", uploader("vfx").any(), (req, res) => {
  res.json({ ok: true, success: true, added: (req.files || []).map((f) => f.filename), vfx: listLibrary("vfx") });
});

router.post("/assets/sfx", uploader("sfx").any(), (req, res) => {
  res.json({ ok: true, success: true, added: (req.files || []).map((f) => f.filename), sfx: listLibrary("sfx") });
});
